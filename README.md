...
- 每 12 小时发现新达标视频，每天续扫最多 3 个最近 90 天的历史时间片。
...
- `412` 限流时延期当前单元并持久化暂停全部B站扫描至少 12 小时；48小时内再次触发则暂停 24 小时。不登录、不保存 Cookie、不绕过验证码。
...
- `BVCollector-Incremental`：每 12 小时增量采集并续跑最多 3 个首次回溯单元。
- `BVCollector-Daily`：每天 03:30 续扫最多 3 个历史单元。
...
B站返回 `412` 或 `v_voucher` 时，当前扫描会保留到“关键词 + 7 天时间片”的断点并立即停止本批B站请求。冷却状态保存在 SQLite，首次触发暂停 12 小时，48 小时内再次触发暂停 24 小时；冷却期间计划任务仍可同步飞书，但会跳过B站请求。热门时间片达到接口 1,000 条上限时仍会自动二分。每 12 小时任务先执行增量扫描，再续跑最多 3 个首次回溯单元；每日 03:30 同样只续扫最多 3 个历史单元。

默认请求节奏为每次请求随机等待 30～45 秒、切换关键词等待 90～150 秒、每连续 10 次请求额外冷却 10 分钟。每 12 小时的增量窗口回看 14 小时，保留 2 小时重叠以防边界遗漏。
